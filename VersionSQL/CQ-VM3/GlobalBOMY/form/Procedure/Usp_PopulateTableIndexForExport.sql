/****** Object:  Procedure [form].[Usp_PopulateTableIndexForExport]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [form].[Usp_PopulateTableIndexForExport]
	@inumFormId int,
	@iExportFormName VARCHAR(50) = 'export.Tb_FormData_'
AS
/*
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @LogMsg varchar(8000) = '',
				...
				@Prefix varchar(50) = 'form.Usp_PopulateTableIndexForExport',
				@Enabled tinyint = 1;
		...
		insert into #fields
		select ComponentTitle,ComponentID, ComponentLabel, 0 from CQBuilder.form.Udf_GetFields(@inumFormId, 1)

		declare @fieldId varchar(4000)
		declare @fieldLabel varchar(4000)

		SET @LogMsg = 'Adding index';
		EXEC [form].[Usp_Logger] @LogMsg, @LastExec OUTPUT, @Prefix, @Enabled;

		while(exists(select 1 from #fields where [Done] = 0))
		begin
			select top 1 @fieldId = ComponentID, @fieldLabel = case when isnull(ComponentLabel,'') <> '' then ComponentLabel else ComponentID end  from #fields where [Done] = 0

			--set @fieldId = (select top 1 ComponentID  from #fields where [Done] = 0);
			set @query = 'ALTER TABLE '+ @tblName + '
						  ADD ['+ CQBuilder.form.Udf_FilterStr(@fieldLabel) + ' (' + @fieldId +  ')] NVARCHAR(4000)';
