/****** Object:  Procedure [reports].[Usp_PopulateReportsDataByReportIDsNotUsed]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [reports].[Usp_PopulateReportsDataByReportIDsNotUsed]
(
	@ReportIDs varchar(200)
)
AS
BEGIN
	SET NOCOUNT ON;
	...
	where IDD in (select value from string_split(@ReportIDs, ','))  --and idd IN(253,254,260)
