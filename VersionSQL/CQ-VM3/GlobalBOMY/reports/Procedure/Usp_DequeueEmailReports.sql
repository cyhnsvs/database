/****** Object:  Procedure [reports].[Usp_DequeueEmailReports]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [reports].[Usp_DequeueEmailReports]
	@batchSize int,
	@ip varchar(50),
...
BEGIN
	...
	DECLARE @istrCCtype VARCHAR(100);
	...
	@istrCCtype = a.value('(CC/@value)[1]', 'nvarchar(max)')
...
select Pr.IDD ProcessID ,'sriram@phillip.com.sg','',S.Name, 
		CASE WHEN(@icharAttachReport='1') THEN S.DestinationPath+S.FileNameSetup ELSE NULL END,@istrEmailBody, PrP.ParamValue RecipientId , prp.ParamName RecipientType
from reports.Tb_ReportSetup S 																							
inner join reports.Tb_Process Pr on Pr.ReportID=S.IDD
inner join reports.Tb_ProcessParam PrP on Pr.IDD=PrP.ProcessID
 Where S.SendEmail=1 and PrP.ParamName in ('istrAcctNo', 'istrMRCode') and pr.IDD in (select processId from @process);
 
 --BETTER COMMENT IT FOR TESTING, IT FETCHES CLIENT/DEALER DETAILS
 ...
 ELSE IF(@istrRecepientType = 'MRCode')
 BEGIN
 UPDATE T
	SET T.Recipient = [WorkEmail (textinput-14)]
	FROM #Email T
	INNER JOIN CQBTempDB.export.Tb_FormData_1575 A
		ON t.RecipientId= A.[MRCode (textinput-17)]
 ...
 IF(@istrCCtype = 'Remisier/Dealer')
 BEGIN
 UPDATE T
	SET T.CC = D.[WorkEmail (textinput-20)]
	FROM #Email T
	INNER JOIN CQBTempDB.export.Tb_FormData_1409 A 
		ON t.RecipientId=A.[AccountNumber (textinput-5)] 
	INNER JOIN
 ...
 */
 ...
 BEGIN
	UPDATE T
	SET T.Recipient= 'nishanth@phillip.com.sg,sriram@phillip.com.sg,nathiya.palanisamy@phillip.com.sg'
	FROM #Email T
 END
 ELSE IF(@istrRecepientType = 'AcctNo')
 BEGIN
	 UPDATE T
		SET T.Recipient= 'michellehoong@msec.com.my,jamielim@msec.com.my',
			T.CC = 'nishanthc@cyberquote.com.sg' --sriram@phillip.com.sg
		FROM #Email T
 ...
 BEGIN
 UPDATE T
	SET T.Recipient= 'nishanth@phillip.com.sg,sriram@phillip.com.sg'
	FROM #Email T
 ...
 UPDATE T
	SET T.CC= 'nathiya.palanisamy@phillip.com.sg'
	FROM #Email T
