/****** Object:  Procedure [reports].[Usp_PopulateReportsData]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [reports].[Usp_PopulateReportsData]
@iMode varchar(20)='',
@ReportIDs varchar(200) = null
-- EXEC [reports].[Usp_PopulateReportsData] 'initial'
-- EXEC [reports].[Usp_PopulateReportsData] '' , '267'
AS
BEGIN
	SET NOCOUNT ON;
	...
	if(@iMode='initial')
	BEGIN
				INSERT INTO [GlobalBOMY].[reports].[Tb_Process_Archive]
					([IDD],[ReportID],[Status],[RetryCount],[MaxRetry],[ScheduleDate],[ProcessByIP],[ProcessByInstanceName]
					  ,[ProcessByThread],[ProcessStarted],[DependencyReportID],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate]
					  ,[Remarks],[ArchivedTime])
				SELECT [IDD],[ReportID],[Status],[RetryCount],[MaxRetry],[ScheduleDate],[ProcessByIP],[ProcessByInstanceName]
					  ,[ProcessByThread],[ProcessStarted],[DependencyReportID],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate]
					  ,[Remarks], GETDATE()
				FROM [GlobalBOMY].[reports].[Tb_Process];

				INSERT INTO [GlobalBOMY].[reports].[Tb_ProcessParam_Archive]
					([IDD],[ProcessID],[ParamName],[ParamValue],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate],[ArchivedTime])
				SELECT [IDD],[ProcessID],[ParamName],[ParamValue],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate], GETDATE()
				FROM [GlobalBOMY].[reports].[Tb_ProcessParam];

				truncate table reports.Tb_Process;
				truncate table reports.Tb_ProcessParam;
	END
	ELSE
	BEGIN
		...
		--IF ((SELECT ProcessStatus FROM Tb_GBOSGProcessToken WHERE IDD=4) = '2'
		--	AND @dteBusinessDate = '2020-03-01')
		--BEGIN
		--	UPDATE reports.Tb_ReportSetup
		--	SET ProcessTokenID=''
		--	WHERE IDD>=240 and IDD<=247
		--END
				...
				select 
						IDD, ScheduleConfig, LastExecutionDate, 0 as Done 
				FROM 
					reports.Tb_ReportSetup A 
				where 
					--token checking
					not exists (select 1 from  reports.Tb_ReportSetup as r 
						  cross apply string_split(ISNULL(r.ProcessTokenID,''), ',') as d
						  left join dbo.Tb_GBOSGProcessToken as pro
						  on d.Value = pro.IDD

						  where r.IDD = a.IDD and ((isnull(pro.ProcessStatus,2) = 0 and @isHoliday=0) or (isnull(pro.ProcessStatus,2) = 0 and @isHoliday=1 AND pro.PHIndicator='Y')))
					and 
					[Enabled] = 1 
					AND (
							iSNULL(@ReportIDs,'')='' 
							...
							(
								iSNULL(@ReportIDs,'')<>'' 
									and  
								IDD in (select value from string_split(@ReportIDs, ','))
							)
						)
					and 
					A.IDD not in (select ReportID from reports.Tb_Process);
				...
				set NextScheduleDate = [reports].[Udf_GetNextSchedule](ScheduleConfig, LastExecutionDate, @isHoliday);
	
				--update #Tb_ReportSetup
				--set NextScheduleDate = getdate()
				--IF ((SELECT ProcessStatus FROM Tb_GBOSGProcessToken WHERE IDD=4) = '2' AND @dteBusinessDate = '2020-03-01')
				--BEGIN 
					--update #Tb_ReportSetup
					--set NextScheduleDate = getdate()
					--WHERE RptIDd=267
				...
				BEGIN  
						...
						BEGIN  
							...
							BEGIN
									SET @sql1 = 'IF OBJECT_ID(''tempdb.dbo.#Tb_ReportProc'', ''U'') IS NOT NULL Drop TABLE #Tb_ReportProc ' + REPLACE (REPLACE (@sql1 ,'@column@', ' ,''9'' AS ProcStatus into #Tb_ReportProc') ,', @TABLE@', '' )
						...
						ELSE
						BEGIN	
						set @sql2 = 'INSERT INTO [reports].[Tb_Process] ([IDD], [ReportID],[ScheduleDate],[Status],[CreatedBy],[CreatedDate]) select (SELECT ISNULL(MAX(IDD),0) + 1 FROM REPORTS.Tb_Process), [RptIDD],''@ScheduleDate'' ,9, ''SYSTEM'', getdate() from #Tb_ReportSetup where [RptIDD] = ''@RptIDD@''';
