/****** Object:  Procedure [dbo].[Usp_GetTierGroupSharing]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE dbo.Usp_GetTierGroupSharing
    @iintCompanyId int
AS
BEGIN
...
SET NOCOUNT ON;
	...
	SELECT CAST(TierGroupId as varchar(20))+'-'+TierGroupCd ItemValue,
		   ...
		   CAST('<tr><th>TierCategory</th><th>FromValue</th><th>ToValue</th><th>Rate</th></tr>' + 
				(select replace (REPLACE(STUFF((select '<tr>' +'<td>' + cast ( TierCategory as varchar) + '</td>' 
													...
													+'<td class=''r''>' + cast ( CAST(ROUND(ToValue,2) as decimal(24,2)) as varchar) + '</td>' 
	...
	WHERE TierCategory='AESharing' and CompanyId = @iintCompanyId;
