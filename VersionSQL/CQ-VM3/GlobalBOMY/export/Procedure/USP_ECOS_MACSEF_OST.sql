/****** Object:  Procedure [export].[USP_ECOS_MACSEF_OST]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [export].[USP_ECOS_MACSEF_OST]
(
	@idteProcessDate DATE
)
AS
-- ONLINE CLIENT - CLIENT OUTSTANDING 
-- EXEC [export].[USP_ECOS_MACSEF_OST] '2020-06-01'
BEGIN
	...
	SELECT 
		...
		CO.AccruedInterestAmountSetl,
		CO.TradedQty,
		CO.NetAmountSetl,
		CO.TradedQty - RemainingQty,
		CO.NetAmountSetl - Balance
	FROM	
	...
	INNER JOIN 
		CQBTempDB.export.Tb_FormData_1409 Account ON Account.[AccountNumber (textinput-5)] =  CO.AcctNo
	LEFT JOIN 
		[GlobalBOLocal].[transmanagement].[Tb_TransactionsSettledUnpaid] T ON CO.TransNo = LEFT(T.ContractNo,10)
	WHERE
		[MRIndicator (multipleradiosinline-4)] LIKE '%M+%' AND ISNULL([CDSNo (textinput-19)],'') <> ''
	AND
		I.InstrumentCd LIKE '%.XKLS%' AND ContractDate = @idteProcessDate
	...
	SELECT 
		RecordType + '|' + ClientCode + '|' +  StockCode + '|' +  TransactionDate + '|' +  ContractNo + '|' +  TransType + '|' +  CONVERT(CHAR(9),Quantity) + '|' +  CONVERT(CHAR(10),Price)
		 + '|' +  CONVERT(CHAR(13),NetAmount)  + '|' +  DueDate + '|' +  CONVERT(CHAR(9),IntFee) + '|' +  CONVERT(CHAR(9),OrgQty) + '|' +  CONVERT(CHAR(13),OrgTransAmount)
