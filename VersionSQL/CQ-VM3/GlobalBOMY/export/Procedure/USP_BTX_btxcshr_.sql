/****** Object:  Procedure [export].[USP_BTX_btxcshr_]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE  [export].[USP_BTX_btxcshr_]
AS
BEGIN
--BATCH HEADER
...
VALUES(1 ,1 ,'CDSNo' ,'ExchangeCo' ,'StockCode' ,1 ,1 ,1 )
--BATCH TRAILER
 DECLARE @Count INT;
...
SELECT RIGHT(REPLICATE('0', 1) + CAST(RecordType as varchar(1)), 1)+RIGHT(REPLICATE('0', 3) + CAST(BranchCode as varchar(3)), 3)+CDSNo+ExchangeCode+StockCode+RIGHT(REPLICATE('0', 13) + CAST(OpeningFreeQty as varchar(13)), 13)+RIGHT(REPLICATE('0', 13) + CAST(OpeningPurchaseQty as varchar(13)), 13)+RIGHT(REPLICATE('0', 13) + CAST(OpeningSalesQty as varchar(13)), 13)  FROM #Detail
UNION ALL

SELECT RIGHT(REPLICATE('0', 1) + CAST(RecordType as varchar(1)), 1)+RIGHT(REPLICATE('0', 6) + CAST(Totalnumberofrecords as varchar(6)), 6)+ProcessingDate+RIGHT(REPLICATE('0', 6) + CAST(BatchNumber as varchar(6)), 6)+Filler  FROM #Trailer
