/****** Object:  Procedure [import].[Usp_AB_ProcessCDSTradingLimit_DailySync]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [import].[Usp_AB_ProcessCDSTradingLimit_DailySync]
AS
/***********************************************************************************             
            
Name              : import.Usp_AB_ProcessCDSTradingLimit_DailySync     
Created By        : Nathiya Palanisamy
Created Date      : 28/12/2020    
Last Updated Date :             
Description       : this sp is used to import CDS Account's Trading Limit from Alliance Bank to GBO
...
Used By : 
EXEC [import].[Usp_AB_ProcessCDSTradingLimit_DailySync]
************************************************************************************/
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (SELECT 1 FROM GlobalBOMY.[import].[Tb_Alliance_AvaiBal]) 
        BEGIN
			...
			set @query = @query + ' INNER JOIN GlobalBOMY.[import].[Tb_Alliance_AvaiBal] AS S
			                        ON RIGHT(S.CDSAcctNo,9) = F.[AccountNumber (textinput-5)]
									WHERE S.TradingLimit <> F.[ApproveTradingLimit (textinput-54)]'
			...
			UPDATE	I
			SET		I.[ApproveTradingLimit (textinput-54)] = A.TradingLimit
			FROM 
				CQBTempDB.[import].Tb_FormData_1409	I
			INNER JOIN 
				[GlobalBOMY].[import].[Tb_Alliance_AvaiBal]	 A ON I.[CDSNo (textinput-19)] = RIGHT(A.CDSAcctNo,9)
			INNER JOIN
				form.Tb_ExportFromDataDelta_1409 D  ON D.[CDSNo (textinput-19)] = RIGHT(A.CDSAcctNo,9)
