/****** Object:  Procedure [import].[Usp_AcctOpeningResponse_UpdateCDSNo_DailySync_20211001]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [import].[Usp_AcctOpeningResponse_UpdateCDSNo_DailySync_20211001]
AS
/***********************************************************************************             
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF EXISTS (SELECT 1 FROM GlobalBOMY.[import].[Tb_AcctOpening_CFT040]) 
        BEGIN
...
set @query = 'insert into form.Tb_ExportFormDataDelta_1409 select DISTINCT RecordID, ''AutoExport'', GETDATE(), NULL, NULL, '
   ...
   set @query = @query + '[' + @fieldId +'], ';
			...
			set @query = @query + ' FROM ' + @reftblName + ' AS C';
			set @query = @query + 'INNER JOIN CQBTempDB.import.Tb_FormData_1410 A  ON A.[CustomerID (textinput-1)] = C.[CustomerID (selectsource-1)]
								   INNER JOIN [GlobalBOMY].[import].[Tb_AcctOpening_CFT040] B  ON A.[CustomerName (textinput-3)] = B.InvestorName
								   WHERE RecordStatus="S" AND ISNULL([CDSNo (textinput-19)],'') = '''
			...
			UPDATE	I
			SET		I.[CDSNo (textinput-19)] = B.AcctNo
			FROM 
				CQBTempDB.[import].Tb_FormData_1409	I
			INNER JOIN CQBTempDB.import.Tb_FormData_1410 A  ON A.[CustomerID (textinput-1)] = I.[CustomerID (selectsource-1)]
			INNER JOIN [GlobalBOMY].[import].[Tb_AcctOpening_CFT040] B  ON A.[CustomerName (textinput-3)] = B.InvestorName
			WHERE RecordStatus='S' AND ISNULL([CDSNo (textinput-19)],'') = ''
