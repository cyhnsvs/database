/****** Object:  Procedure [process].[Usp_Daily_MarginFacilityRenewal]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [process].[Usp_Daily_MarginFacilityRenewal]
	@iintCompanyId BIGINT,
    @ostrReturnMessage VARCHAR(4000) OUTPUT
AS
/*********************************************************************************** 

Name              : process.[Usp_Daily_MarginFacilityRenewal]
Created By        : Nishanth
...
Used By :
Exec [process].[Usp_Daily_MarginFacilityRenewal] 1, ''
...
BEGIN
	...
	SET @dteNextBusinessDate = GlobalBO.[global].[Udf_GetNextBusDateByDaysExcludePH](@dteBusinessDate,1,NULL);
		...
		SET		FormDetails = JSON_MODIFY(
								JSON_MODIFY(
									JSON_MODIFY(
										JSON_MODIFY(
											FormDetails,
											'$[0].dateinput4',CAST(DATEADD(day, 90, CAST([dateinput-4] as date)) AS VARCHAR)),
											'$[1].dateinput4',CAST(DATEADD(day, 90, CAST([dateinput-4] as date)) AS VARCHAR)),
											'$[0].dateinput5',CAST(DATEADD(day, 90, CAST([dateinput-5] as date)) AS VARCHAR)),
											'$[1].dateinput5',CAST(DATEADD(day, 90, CAST([dateinput-5] as date)) AS VARCHAR))
		FROM	CQBuilder.form.Tb_FormData_1409 A
		WHERE	[selectsource-3] = 'M' 
		AND		[dateinput-5] >= @dteBusinessDate AND [dateinput-5] < @dteNextBusinessDate
        
		UPDATE	A
		SET		[CommencementDate (dateinput-4)] = DATEADD(day, 90, CAST([CommencementDate (dateinput-4)] as date)),	--@dteBusinessDate,
				[TenorExpiryDate (dateinput-5)]  = DATEADD(day, 90, CAST([TenorExpiryDate (dateinput-5)] as date))	--DATEADD(day, 90, @dteBusinessDate)	
