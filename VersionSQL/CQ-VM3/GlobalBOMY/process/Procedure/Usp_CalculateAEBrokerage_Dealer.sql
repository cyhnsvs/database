/****** Object:  Procedure [process].[Usp_CalculateAEBrokerage_Dealer]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [process].[Usp_CalculateAEBrokerage_Dealer]
...
Name              : [process].[Usp_CalculateAEBrokerage_Dealer]
...
Used By :
Exec [process].[Usp_CalculateAEBrokerage_Dealer] 1, ''
...
BEGIN
  ...
  --          ,[WHTaxJuristic],[OtherCollateral],[ART],[ARTMinus1],[APT],[ATSFeeT]
   ...
   --     INNER JOIN GlobalBO.setup.Tb_Instrument AS K ON
		...
		SELECT LogDateTime, 'GlobalBOMY.process.Usp_CalculateAEBrokerage_Dealer', '', [MessageLog] 
		from @logs;
	...
	rollback tran;
		...
		SET @strEmailSubj = @strEmailSubj + ' - Usp_CalculateAEBrokerage_Dealer: Failed'
