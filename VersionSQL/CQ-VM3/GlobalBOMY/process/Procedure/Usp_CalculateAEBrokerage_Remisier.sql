/****** Object:  Procedure [process].[Usp_CalculateAEBrokerage_Remisier]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [process].[Usp_CalculateAEBrokerage_Remisier]
...
Name              : [process].[Usp_CalculateAEBrokerage_Remisier]
...
Used By :
Exec [process].[Usp_CalculateAEBrokerage_Remisier] 1, ''
...
BEGIN
	...
	begin tran
		...
		(
			...
			SetlCurrCd VARCHAR(20),
			ClientBrokerageSetl decimal(24,9),
			IndividualIncentiveAmount decimal(24,9) DEFAULT (0),
  ...
  --          ,[WHTaxJuristic],[OtherCollateral],[ART],[ARTMinus1],[APT],[ATSFeeT]
		...
		WHERE [DealerType (selectsource-3)] IN ('R');

		SELECT *
		INTO #Contracts
		FROM (
			SELECT CO.AcctExecutiveCd, R.[BFEDealerCode (textinput-26)], CO.ExchCd, InstrumentCd, BG.TierGroupId, TG.TierGroupCd, CO.SetlCurrCd, CO.ClientBrokerageSetl, 
					SUBSTRING(CO.Tag5,0,CHARINDEX('|',CO.Tag5)) AS AcctExecutiveCdMR, SUBSTRING(CO.Tag5,CHARINDEX('|',CO.Tag5)+1,LEN(CO.Tag5)) AS BranchId
			...
			INNER JOIN #Remisiers AS R
			ON CO.AcctExecutiveCd = R.[DealerCode (textinput-35)]
		...
		--SELECT @dteEOM, AcctExecutiveCd, C.BranchId, ExchCd, InstrumentCd, SetlCurrCd, SUM(ClientBrokerageSetl) AS ClientBrokerageSetl, 'IndForeignShares' AS IncentiveType
		--FROM #Contracts AS C
		----INNER JOIN CQBTempDB.export.Tb_FormData_1377 AS D
		----ON C.AcctExecutiveCd = D.[DealerCode (selectsource-14)]
		--WHERE ExchCd <> 'XKLS'
		--GROUP BY AcctExecutiveCd, C.BranchId, ExchCd, InstrumentCd, SetlCurrCd;
		
		-- Tier Group
		INSERT INTO #AEBrokerageDetailed
			(BusinessDate, AcctExecutiveCd, BranchId, TierGroupId, TierGroupCd, SetlCurrCd, ClientBrokerageSetl, IncentiveType)
		SELECT @dteEOM, AcctExecutiveCd, C.BranchId, C.TierGroupId, C.TierGroupCd, SetlCurrCd, SUM(ClientBrokerageSetl) AS ClientBrokerageSetl, 'IndTierGroup' AS IncentiveType
		FROM #Contracts AS C
		...
		GROUP BY AcctExecutiveCd, C.BranchId, TierGroupId, TierGroupCd, SetlCurrCd;
		
		-- Normal
		INSERT INTO #AEBrokerageDetailed
			(BusinessDate, AcctExecutiveCd, BranchId, SetlCurrCd, ClientBrokerageSetl, IncentiveType)
		SELECT @dteEOM, AcctExecutiveCd, C.BranchId, SetlCurrCd, SUM(ClientBrokerageSetl) AS ClientBrokerageSetl, 'IndNormal' AS IncentiveType
		FROM #Contracts AS C
		LEFT JOIN CQBTempDB.export.Tb_FormData_1680 AS TGS
		ON CAST(C.TierGroupId as varchar(30)) + C.TierGroupCd = TGS.[BrokerageTier (selectbasic-1)]
		WHERE TGS.RecordID IS NULL --ExchCd = 'XKLS' OR 
		GROUP BY AcctExecutiveCd, C.BranchId, SetlCurrCd;
		...
		FROM #Contracts AS C
		INNER JOIN CQBTempDB.export.Tb_FormData_1377 AS D
		ON C.AcctExecutiveCd = D.[DealerCode (textinput-35)]
		INNER JOIN GlobalBO.setup.Tb_TierGroup AS TG
		...
		INSERT INTO #AEBrokerageDetailed
			(BusinessDate, AcctExecutiveCd, BranchId, SetlCurrCd, OtherBranchId, ClientBrokerageSetl, IncentiveType, IncentiveTierGroupId, IncentiveTierGroupCd)
		SELECT @dteEOM, D.[DealerCode (textinput-35)], D.[BranchID (selectsource-1)], SetlCurrCd, C.BranchId, C.TotalBrokerage, 'KioskAdd' AS IncentiveType, TG.TierGroupId, TG.TierGroupCd
		FROM #BranchSummary AS C
		INNER JOIN 
			(SELECT *
			 FROM CQBTempDB.export.Tb_FormData_1377 AS D
			 CROSS APPLY OPENJSON (D.[AdditionalIncentiveForOtherBranch (grid-2)])
			 WITH (
				RowIndex bigint '$.rowIndex',
				BranchId varchar(10) '$.seq1',
				IncentiveRate varchar(50) '$.seq2'
				) as F
			 WHERE D.[AdditionalIncentiveForOtherBranch (grid-2)] <> ''
			) AS D
		ON C.BranchId = D.BranchId
		INNER JOIN GlobalBO.setup.Tb_TierGroup AS TG
		ON D.IncentiveRate = CAST(TG.TierGroupId as varchar(30)) + '-' + TG.TierGroupCd;

		-- MR
		INSERT INTO #AEBrokerageDetailed
			(BusinessDate, AcctExecutiveCd, AcctExecutiveCdMR, BranchId, SetlCurrCd, ClientBrokerageSetl, IncentiveType, IncentiveTierGroupId, IncentiveTierGroupCd)
		SELECT @dteEOM, AcctExecutiveCd, AcctExecutiveCdMR, C.BranchId, SetlCurrCd, SUM(ClientBrokerageSetl) AS ClientBrokerageSetl, 'MRNormal' AS IncentiveType, TG.TierGroupId, TG.TierGroupCd
		FROM #Contracts AS C
		...
		FROM #AEBrokerageDetailed AS AB
		INNER JOIN CQBTempDB.export.Tb_FormData_1377 AS D
		ON AB.AcctExecutiveCd = D.[DealerCode (textinput-35)]
		INNER JOIN GlobalBO.setup.Tb_TierGroup AS TG
   ...
   --     INNER JOIN GlobalBO.setup.Tb_Instrument AS K ON
		...
		INSERT INTO #AEBrokerageSummary
			(BusinessDate, AcctExecutiveCd, BranchId, SetlCurrCd, ClientBrokerageSetl, IndividualIncentiveAmount)
		...
		GROUP BY BusinessDate, AcctExecutiveCd, BranchId, SetlCurrCd;

		UPDATE AES
		SET KioskIncentiveAmount = CASE WHEN IndividualIncentiveAmount - AD.IncentiveAmount <= 0 THEN 0 ELSE IndividualIncentiveAmount - AD.IncentiveAmount END
		...
		INSERT INTO [dbo].[AEIncentive_Remisiers_Details]
           ([BusinessDate],[AcctExecutiveCd],[AcctExecutiveCdMR],[BranchId],[TierGroupId]
           ,[TierGroupCd],[OtherBranchId],[SetlCurrCd],[ClientBrokerageSetl]
           ,[IncentiveType],[IncentiveTierGroupId],[IncentiveTierGroupCd],[IncentiveAmount])
		SELECT [BusinessDate],[AcctExecutiveCd],[AcctExecutiveCdMR],[BranchId],[TierGroupId]
           ,[TierGroupCd],[OtherBranchId],[SetlCurrCd],[ClientBrokerageSetl]
           ,[IncentiveType],[IncentiveTierGroupId],[IncentiveTierGroupCd],[IncentiveAmount] 
		FROM #AEBrokerageDetailed;

		INSERT INTO [dbo].[AEIncentive_Remisiers_Summary]
           ([BusinessDate],[AcctExecutiveCd],[BranchId],[SetlCurrCd],[ClientBrokerageSetl]
           ,[IndividualIncentiveAmount],[KioskIncentiveAmount],[AdditionalKioskIncentiveAmount])
		SELECT [BusinessDate],[AcctExecutiveCd],[BranchId],[SetlCurrCd],[ClientBrokerageSetl]
           ,IndividualIncentiveAmount,[KioskIncentiveAmount],[AdditionalKioskIncentiveAmount]
		FROM #AEBrokerageSummary;

		INSERT INTO [dbo].[AEIncentive_Remisiers_MR_Summary]
           ([BusinessDate],[AcctExecutiveCd],[AcctExecutiveCdMR],[BranchId],[SetlCurrCd]
           ,[ClientBrokerageSetl],[IncentiveAmount])
		SELECT [BusinessDate],[AcctExecutiveCd],[AcctExecutiveCdMR],[BranchId],[SetlCurrCd]
           ,[ClientBrokerageSetl],[IncentiveAmount]
		...
		SELECT LogDateTime, 'GlobalBOMY.process.Usp_CalculateAEBrokerage_Remisier', '', [MessageLog] 
		from @logs;
	...
	rollback tran;
		...
		SET @strEmailSubj = @strEmailSubj + ' - Usp_CalculateAEBrokerage_Remisier: Failed'
