/****** Object:  Procedure [process].[Usp_CalculateAEBrokerage_DealerQuarterly]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [process].[Usp_CalculateAEBrokerage_DealerQuarterly]
...
Name              : [process].[Usp_CalculateAEBrokerage_DealerQuarterly]
...
Used By :
Exec [process].[Usp_CalculateAEBrokerage_DealerQuarterly] 1, ''
...
BEGIN
  ...
  --          ,[WHTaxJuristic],[OtherCollateral],[ART],[ARTMinus1],[APT],[ATSFeeT]
		...
		CROSS APPLY OPENJSON ([EffortSharing (grid-1)])
			 WITH (
				RowIndex bigint '$.rowIndex',
				QuarterNo varchar(10) '$.seq1',
				DealerCode varchar(50) '$.seq2',
				PercentageShared varchar(50) '$.seq3'
				) as F
		
		SELECT * 
		INTO #IncentiveContributors
		FROM (
			SELECT RecordID, [Region (selectsource-2)], [BranchID (selectsource-1)], [TeamLeader (selectbasic-1)], value as DealerCode
			FROM CQBTempDB.export.Tb_FormData_1710 AS TS
			CROSS APPLY string_split([IncentiveContributors (selectsourcemultiple-1)], ',')
			UNION
			SELECT RecordID, [Region (selectsource-2)], [BranchID (selectsource-1)], [TeamLeader (selectbasic-1)], '' as DealerCode
			FROM CQBTempDB.export.Tb_FormData_1710 AS TS
			WHERE [IncentiveContributors (selectsourcemultiple-1)] IS NULL) AS D;
   ...
   --     INNER JOIN GlobalBO.setup.Tb_Instrument AS K ON
		...
		--Dealer Sharing
		SELECT *
		FROM CQBTempDB.export.Tb_FormData_1696
		...
		SELECT LogDateTime, 'GlobalBOMY.process.Usp_CalculateAEBrokerage_DealerQuarterly', '', [MessageLog] 
		from @logs;
	...
	rollback tran;
		...
		SET @strEmailSubj = @strEmailSubj + ' - Usp_CalculateAEBrokerage_DealerQuarterly: Failed'
