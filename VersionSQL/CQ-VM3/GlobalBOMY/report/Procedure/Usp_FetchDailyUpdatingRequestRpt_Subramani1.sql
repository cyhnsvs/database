/****** Object:  Procedure [report].[Usp_FetchDailyUpdatingRequestRpt_Subramani1]    Committed by VersionSQL https://www.versionsql.com ******/

/***********************************************************************************             
Name              : [report].[Usp_FetchDailyUpdatingRequestRpt_Subramani1]
Created By        : Fadlin    
...
Modification History :            
 ...
 EXEC [report].[Usp_FetchDailyUpdatingRequestRpt_Subramani1] '2020-12-09'
**********************************************************************************/   
CREATE PROCEDURE [report].[Usp_FetchDailyUpdatingRequestRpt_Subramani1]
	@idteReportDate date
AS
BEGIN
	BEGIN TRY 

		DECLARE @strColName nvarchar(max), @strColNameWithoutAcctNo nvarchar(max);
		...
		SELECT @strColNameWithoutAcctNo = STRING_AGG (ColName, ',') FROM #Col1409 WHERE ColName NOT IN ('[AccountNumber (textinput-5)]','[RecordID]','[CreatedBy]','[CreatedTime]','[UpdatedBy]','[UpdatedTime]');--ColName <> '[AccountNumber (textinput-5)]';
		SELECT @strColName = STRING_AGG (ColName, ',') FROM #Col1409 WHERE ColName NOT IN ('[RecordID]','[CreatedBy]','[CreatedTime]','[UpdatedBy]','[UpdatedTime]');
		...
		SET @query = '
			...
			(
				...
				FROM
				(
					SELECT ROW_NUMBER() OVER (PARTITION BY '+ @strColName +' ORDER BY ReportDate) row_num,ReportDate,'+@strColName+'
		...
		(
			SELECT ReportDate,'+@strColName+' FROM #current1409 WHERE [AccountNumber (textinput-5)] IN (select AcctNo FROM #diff1409)
			UNION ALL
  ...
  --  WHERE [object_id] = OBJECT_ID(N'tempdb.dbo.#current1409');
  ...
  --  ON c.system_type_id = t.system_type_id
  --  AND t.system_type_id = t.user_type_id
  --  WHERE [object_id] = OBJECT_ID(N'tempdb.dbo.#old1409');
				--select * from #old1409

		--PRINT 'IN_1'

		--	SET @query = 'SELECT * INTO #diffData1409 FROM
		--(
		--	SELECT ReportDate,'+@strColName+' FROM #current1409 WHERE [AccountNumber (textinput-5)] IN (select AcctNo FROM #diff1409)
		--	UNION ALL
		--	SELECT * FROM #old1409 WHERE [AccountNumber (textinput-5)] IN (select AcctNo FROM #diff1409)
