/****** Object:  Procedure [Queue].[AddMessageToOutBoundQueue_20210923]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [Queue].[AddMessageToOutBoundQueue_20210923] (
...
@Protocol_ETX char(1)=3 --<ETX>
	...
	+ '|#~' + '30' + '|#~' +'032'  + '|602'										
	+ '|#~' + '31' + '|#~' +'130'  + '|##QUEUE_ID##'
	+'|#~'+'31'+'|#~'+'051'+'|'+		CAST(ISNULL(A.[AccountNumber (textinput-5)],'') as varchar(09) )	--ClientNumber
	+'|#~'+'31'+'|#~'+'133'+'|'+		CAST(ISNULL(A.[AccountNumber (textinput-5)],'') as varchar(10) )	--ClientShortName
	+'|#~'+'31'+'|#~'+'098'+'|'+		CAST(ClientName as varchar(30) )	
	+'|#~'+'31'+'|#~'+'135'+'|'+		CAST(ClientAddress as varchar(160) )	
	+'|#~'+'31'+'|#~'+'150'+'|'+		CAST(TelephoneNumber as varchar(12) )	
	+'|#~'+'31'+'|#~'+'047'+'|'+		RIGHT(REPLICATE('0',9) + CAST(ISNULL(A.[CDSNo (textinput-19)],'') as varchar(09) ),9)
	+'|#~'+'31'+'|#~'+'100'+'|'+		CAST(ClientICNumber as varchar(14) )	
	+'|#~'+'31'+'|#~'+'186'+'|'+		RIGHT(REPLICATE('0',3) + CAST(ISNULL(D.[BranchID (selectsource-1)],'') as varchar(03) ),3)
	+'|#~'+'31'+'|#~'+'052'+'|'+		RIGHT(REPLICATE('0',5) + CAST(ISNULL(D.[BFEDealerCode (textinput-26)],'') as varchar(05) ),5)
	+'|#~'+'31'+'|#~'+'140'+'|'+		RIGHT(REPLICATE('0',2) + CAST(ISNULL(A.[BFEACType (selectsource-29)],'') as varchar(02) ),2)
	+'|#~'+'31'+'|#~'+'141'+'|'+		CAST(CASE WHEN ISNULL(A.[StructureWarrant (selectbasic-7)],'') = 'Y' THEN 1 ELSE 0 END AS varchar(01) )				--CallWarrantTradingAllowed
	+'|#~'+'31'+'|#~'+'142'+'|'+		CAST(CASE WHEN ISNULL(A.[ClientAssoallowed (multipleradiosinline-13)],'') = 'Y' THEN 1 ELSE 0 END  as varchar(01) )	--ClientAssociateAllowedFlag	
	+'|#~'+'31'+'|#~'+'137'+'|'+		CAST(ISNULL(NULLIF(A.[ShortSellPDTInd (selectbasic-43)],''),0)as varchar(01) )										--ShortSellingTradingAllowed
	+ '|#~' + @Protocol_ETX as Msg,
	9 as [Status]
	FROM OPENJSON(@JsonMessageData)
	WITH (
	...
	ON C.[CustomerID (textinput-1)] = json.ClientId
	LEFT JOIN CQBTempDB.export.Tb_FormData_1409 AS A
	ON A.[CustomerID (selectsource-1)] = C.[CustomerID (textinput-1)]
	LEFT JOIN CQBTempDB.export.Tb_FormData_1377 AS D
	...
	LEFT JOIN CQBTempDB.export.Tb_FormData_1409 A
	 ON A.[AccountNumber (textinput-5)] = json.ClientNumber
	LEFT JOIN CQBTempDB.export.Tb_FormData_1410 AS C
	...
	+ '|#~' + '30' + '|#~' +'032'  + '|613'										
	+ '|#~' + '31' + '|#~' +'130'  + '|##QUEUE_ID##'			 
	+'|#~'+'31'+'|#~'+'186'+'|'+		RIGHT(REPLICATE('0',3) + CAST(D.[BranchID (selectsource-1)] as varchar(03) ),3)
	+'|#~'+'31'+'|#~'+'052'+'|'+		RIGHT(REPLICATE('0',5) + CAST(D.[BFEDealerCode (textinput-26)] as varchar(05) ),5)
	+'|#~'+'31'+'|#~'+'156'+'|'+		CAST(CASE WHEN D.[BranchID (selectsource-1)] = 'Y' THEN 1 ELSE 0 END as varchar )	
	+'|#~'+'31'+'|#~'+'157'+'|'+		RIGHT(REPLICATE('0',12) + CAST(MaxBuyExposureLimit as varchar(12) ),12)
	+'|#~'+'31'+'|#~'+'158'+'|'+		RIGHT(REPLICATE('0',12) + CAST(MaxSellExposureLimit as varchar(12) ),12)
	+'|#~'+'31'+'|#~'+'159'+'|'+		RIGHT(REPLICATE('0',12) + CAST(MaxNettExposureLimit as varchar(12) ),12)
	+'|#~'+'31'+'|#~'+'160'+'|'+		RIGHT(REPLICATE('0',12) + CAST(MaxTotalExposureLimit as varchar(12) ),12)
	+'|#~'+'31'+'|#~'+'161'+'|'+		RIGHT(REPLICATE('0',3)  + CAST(DM.[ExceedLimit (textinput-10)]  as varchar(03) ),3)
	+'|#~'+'31'+'|#~'+'162'+'|'+		CAST(DM.[ClearPreviousDayOrder (multipleradiosinline-2)] as varchar )	
	+ '|#~' + @Protocol_ETX as Msg,
	9 as [Status]
	FROM OPENJSON(@JsonMessageData)
	WITH (
	...
	LEFT JOIN CQBTempDB.export.Tb_FormData_1377 AS D 
	ON D.[DealerCode (textinput-35)] = json.DealerCode
	LEFT JOIN CQBTempDB.export.Tb_FormData_1379 AS DM
	...
	+'|#~'+'31'+'|#~'+'161'+'|'+		RIGHT(REPLICATE('0',3) + CAST(A.[ExceedLimit (textinput-71)] as varchar(03) ),3)
	+'|#~'+'31'+'|#~'+'162'+'|'+		CAST(A.[ClearPreviousDayOrder (multipleradiosinline-19)] as varchar )	
	+ '|#~' + @Protocol_ETX as Msg,
	9 as [Status]
	FROM OPENJSON(@JsonMessageData)
	WITH (
	...
	+'|#~'+'31'+'|#~'+'186'+'|'+		RIGHT(REPLICATE('0',3) + CAST(BranchID as varchar(03) ),3)
	+'|#~'+'31'+'|#~'+'167'+'|'+		CAST(ISNULL(DM.[Access (multipleradiosinline-3)],'') as varchar )		--AccessSuspendResume
	+'|#~'+'31'+'|#~'+'168'+'|'+		CAST(ISNULL(DM.[Buy (multipleradiosinline-4)],'') as varchar )	        --BuySuspendResume
	+'|#~'+'31'+'|#~'+'169'+'|'+		CAST(ISNULL(DM.[Sell (multipleradiosinline-5)],'') as varchar )		--SellSuspendResume
